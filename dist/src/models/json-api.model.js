...
var JsonApiModel = /** @class */ (function () {
        ...
        if (hasOne) {
            for (var _i = 0, hasOne_1 = hasOne; _i < hasOne_1.length; _i++) {
                ...
                if (relationship && relationship.data) {
                    ...
                    var modelType = Reflect.getMetadata('JsonApiDatastoreConfig', this._datastore.constructor).models[typeName];
                    if (modelType) {
                        var modelMetadata = Reflect.getMetadata('JsonApiModelConfig', modelType);
                        var relationshipModel = this.getHasOneRelationship(modelType, relationship.data, included, typeName, level, modelMetadata.type_one);
    ...
    JsonApiModel.prototype.getHasOneRelationship = function (modelType, data, included, typeName, level, type_one) {
        var id = data.id;
